...
Thirty-two times in this project a test looked green because it measured the
...
| 32 | macOS shim suite | the suite could be run twice | its last act is `stop --all`, which stops the VM, and its first asks `doctor` what the VM says about itself — which a stopped one does not. So the second run failed on a check about `doctor` rather than about anything the run had done. It brings the VM up first now, which is what a user's first command does anyway |
