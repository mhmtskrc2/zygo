...
BINARY=tests/linux/bin/zygo-linux-musl-vm
KERNEL=tests/linux/bin/vm-out/Image
...
ssh "$HOST" "mkdir -p ~/$REMOTE/data/backends/krun ~/$REMOTE/tests/linux"

# The checklist first. It refuses when another session is running, and that
# refusal is the point.
scp -q tests/linux/pi_env.sh tests/linux/cgroup_harness.sh tests/linux/verify_vm.sh "$HOST:~/$REMOTE/tests/linux/"
if ! ssh "$HOST" "sh ~/$REMOTE/tests/linux/pi_env.sh"; then
    echo ""
