...
#
#     systemd-run --user --scope -p Delegate=yes -- sh tests/linux/verify_launcher.sh
#
...
clear_data_home() {
    ...
    echo "  over from an earlier run. \`sh tests/linux/cleanup.sh\` clears those." >&2
    return 1
}
...
if mkdir -p "$ZYGO_HARNESS_ROOT/harness" "$ZYGO_HARNESS_ROOT/launch" 2>/dev/null; then
    ...
    # executable bit: the suites are started as `sh tests/linux/<name>.sh`, and one of
    # them is bash.
