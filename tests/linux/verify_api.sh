...
python3 "$SRC/tests/linux/api_driver.py" "$SOCK" "$WORK" "$IMAGE"
status=$?
...
if wait_for "$SOCK_CALL_ONLY"; then
    python3 "$SRC/tests/linux/api_driver.py" --call-only "$SOCK_CALL_ONLY" "$IMAGE" || status=1
else
...
python3 "$SRC/tests/linux/usage_sink.py" "$WORK/usage.port" "$ZYGO_USAGE_SINK" \
    >"$WORK/usage.log" 2>&1 &
USAGE_PID=$!
i=0
...
if wait_for "$SOCK_TOKENS"; then
    python3 "$SRC/tests/linux/api_driver.py" --tokens "$SOCK_TOKENS" "$IMAGE" || status=1
else
...
if wait_for "$SOCK_DRAIN"; then
    python3 "$SRC/tests/linux/api_driver.py" --drain "$SOCK_DRAIN" "$IMAGE" || status=1
else
