...
#   sh tests/linux/build_guest.sh     # from inside the VM
#
# Same artifact as `make tests/linux/bin/zygo-linux-musl`, same name, same static musl
...
# `make tests/linux/bin/zygo-linux-musl` does it: the workspace `[patch]` names a git
...
mkdir -p tests/linux/bin
cp "$CARGO_TARGET_DIR/$TARGET/release/zygo" tests/linux/bin/zygo-linux-musl
sh tests/linux/check_dist.sh tests/linux/bin/zygo-linux-musl
