...
# the evidence for it, and `tests/linux/repro_blue_green.sh` could not reproduce it
...
# running, which is the same rule `tests/linux/cleanup.sh` uses to decide what it may
# kill.
leftover=0
names=
for d in /proc/[0-9]*; do
