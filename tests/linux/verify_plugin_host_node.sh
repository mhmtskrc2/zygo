...
# The Node plugin host, run for real against the same API.
#
# The Node twin of verify_plugin_host.sh: the same claim — a plugin host on
# the HTTP API alone, no `sandbox.toml`, no file on the Zygo host, no shell on
# it — made from the Node SDK, and as an HTTP server of its own. What runs is
# `examples/plugin-host-node/verify.mjs`, which starts the host in-process
# and drives its routes the way a customer would.
#
...
# stores.
#
# Run:  make verify-plugin-host-node
set -u

SRC=${SRC:-/src}
ZYGO=${ZYGO:-$SRC/tests/linux/bin/zygo-linux-musl}
IMAGE=${IMAGE:-python:3.12-slim}
# Two languages, one host: a Python and a JavaScript plugin, under the same
# customer's limits.
...
if [ ! -x "$ZYGO" ]; then
    echo "  no binary at $ZYGO — run \`make tests/linux/bin/zygo-linux-musl\` first" >&2
    exit 1
fi
...
ZYGO_IMAGE=$IMAGE ZYGO_NODE_IMAGE=$NODE_IMAGE \
    node "$SRC/examples/plugin-host-node/verify.mjs" "unix://$SOCK"
status=$?
