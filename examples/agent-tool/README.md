...
and is one-shot only, so it cannot hold a warm function yet; until it can,
this runs on `ns` with the tightest profile it has, and the
[threat model](../../docs/book/23-security.md) says plainly what that does and
